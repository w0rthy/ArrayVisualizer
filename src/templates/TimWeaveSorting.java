...
final public class TimWeaveSorting {
    ...
    public static int getMinRun() {
        return TimWeaveSorting.MIN_MERGE / 2;
    }
     ...
     */
    public TimWeaveSorting(int[] a, int currentLen, Highlights markOps, Reads readOps, Writes writeOps) {
        this.a = a;
        this.len = currentLen;
     ...
     */
    public static void sort(TimWeaveSorting timSort, int[] a, int length) {
        sort(timSort, a, 0, length);
    }
    static void sort(TimWeaveSorting timSort, int[] a, int lo, int hi) {
        TimWeaveSorting ts = timSort;
    ...
    private static void binarySort(TimWeaveSorting ts, int[] a, int lo, int hi, int start) {
        if (start == lo)
            start++;
     ...
     *          the specified array
     */
    private static int countRunAndMakeAscending(TimWeaveSorting ts, int[] a, int lo, int hi) {
        int runHi = lo + 1;
        if (runHi == hi)
            return 1;
     ...
     */
    private static void reverseRange(TimWeaveSorting ts, int[] a, int lo, int hi) {
   ...
   private void mergeCollapse() {
     ...
     *    should follow it.
     */
    private static int gallopLeft(TimWeaveSorting ts, int key, int[] a, int base, int len, int hint) {
        int lastOfs = 0;
        int ofs = 1;
     ...
     * @return the int k,  0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
     */
    private static int gallopRight(TimWeaveSorting ts, int key, int[] a, int base, int len, int hint) {
        int ofs = 1;
        int lastOfs = 0;
        if (ts.Reads.compare(key, a[base + hint]) < 0) {
     ...
     */
    private void mergeLo(TimWeaveSorting ts, int base1, int len1, int base2, int len2) {
     ...
     * @param len2  length of second run to be merged (must be > 0)
     */
    private void mergeHi(TimWeaveSorting ts, int base1, int len1, int base2, int len2) {
