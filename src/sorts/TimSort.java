 ...
 * @author Josh Bloch
 * 
 * Tailored to ArrayVisualizer by John R.
 */
public class TimSort {
    ...
    public static void arraycopy(int[] src, int srcPos, int[] dest, int destPos, int length, boolean temp) {
    	for(int i = 0; i < length; i++) {
    		write(dest, destPos + i, src[srcPos + i], 1, false, temp);
    		if(temp) marked.set(1, srcPos + i);
    		else marked.set(1, dest[destPos + i]);
    		sleep(1);
    	}
    }
    
    public static void reversearraycopy(int[] src, int srcPos, int[] dest, int destPos, int length) {
    	for(int i = length - 1; i >= 0; i--) {
    		write(dest, destPos + i, src[srcPos + i], 1, false, false);
    		marked.set(1, dest[destPos + i]);
    		sleep(1);
    	}
    }
   ...
   private static void binarySort(int[] a, int lo, int hi) {
       partialBinaryInsert(a, lo, hi, 0.5);
       clearmarked();
   }
   ...
   private static void reverseRange(int[] a, int lo, int hi) {
	   for (int left = lo, right = hi - 1; left < right; left++, right--) {
   ...
   private void pushRun(int runBase, int runLen) {
       write(this.runBase, stackSize, runBase, 0, false, true);
       write(this.runLen, stackSize, runLen, 0, false, true);
       stackSize++;
   }
   ...
   private void mergeAt(int i) {
        ...
        */
       write(runLen, i, len1 + len2, 0, false, true);
       if (i == stackSize - 3) {
    	   write(runBase, i + 1, runBase[i + 2], 0, false, true);
           write(runLen, i + 1, runLen[i + 2], 0, false, true);
       }
       stackSize--;
   ...
   private void mergeLo(int base1, int len1, int base2, int len2) {
       ...
       arraycopy(a, base1, tmp, 0, len1, true);
       ...
       write(a, dest++, a[cursor2++], 1, false, false);
       marked.set(1, dest - 1);
       if (--len2 == 0) {
           arraycopy(tmp, cursor1, a, dest, len1, false);
           return;
       }
       if (len1 == 1) {
           arraycopy(a, cursor2, a, dest, len2, false);
           write(a, dest + len2, tmp[cursor1], 1, false, false); // Last elt of run 1 to end of merge
           marked.set(1, dest + len2);
           return;
       }
       ...
       while (true) {
           ...
           do {
               ...
               if (compare(a[cursor2], tmp[cursor1]) < 0) {
            	   ...
            	   write(a, dest++, tmp[cursor1++], 1, false, false);
                   marked.set(1, dest - 1);
                   count1++;
                   count2 = 0;
                   if (--len1 == 1)
           ...
           do {
               ...
               if (count1 != 0) {
                   arraycopy(tmp, cursor1, a, dest, count1, false);
                   dest += count1;
                   cursor1 += count1;
                   len1 -= count1;
               ...
               if (count2 != 0) {
                   arraycopy(a, cursor2, a, dest, count2, false);
                   dest += count2;
                   cursor2 += count2;
                   len2 -= count2;
                   if (len2 == 0)
   ...
   private void mergeHi(int base1, int len1, int base2, int len2) {
       ...
       arraycopy(a, base2, tmp, 0, len2, true);
       ...
       write(a, dest--, a[cursor1--], 1, false, false);
       marked.set(1, dest + 1);
       if (--len1 == 0) {
       ...
       if (len2 == 1) {
           ...
           write(a, dest, tmp[cursor2], 1, false, false);
           marked.set(1, dest);
           return;
       }
   ...
   outer:
       ...
       if (len2 == 1) {
           ...
           cursor1 -= len1;
           reversearraycopy(a, cursor1 + 1, a, dest + 1, len1);
           write(a, dest, tmp[cursor2], 1, false, false); // Move first elt of run2 to front of merge
           marked.set(1, dest);
       } else if (len2 == 0) {
           throw new IllegalArgumentException(
               "Comparison method violates its general contract!");
